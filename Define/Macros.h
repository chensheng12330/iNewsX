...
#define UIColorWithHEX_Alpha(hex, a) ([UIColor colorWithRed:((float)((hex & 0xFF0000) >> 16)) / 255.0 green:((float)((hex & 0xFF00) >> 8)) / 255.0 blue:((float)(hex & 0xFF)) / 255.0 alpha:a])
#define UIColorWithHEX(hex) (UIColorWithHEX_Alpha(hex, 1.0f))
...
#define NSStringSafeExt(string) (ValidString(string) ? string : @"")

#define NSStringContainsString(string, key) ([string rangeOfString:key].location != NSNotFound)


//数组
#define NSArraySafeIndex(array, index) (if (index < array.count) {return array[index];} else return nil;)
...
CG_INLINE void
LTReplaceMethod(Class _class, SEL _originSelector, SEL _newSelector) {
